...
void App::initButtons() {
  ...
  y += btnH + spacing;
  buttons.push_back(
      {"Mid. Circle", 10, y, 110, btnH, AppMode::DRAW_CIRCLE_MIDPOINT});
  buttons.push_back(
      {"Bres. Circle", 130, y, 110, btnH, AppMode::DRAW_CIRCLE_BRESENHAM});
  y += btnH + spacing;
  ...
  buttons.push_back(
      {"Draw Polygon", 10, y, 230, btnH, AppMode::CLIPPING_POLYGON});
  y += btnH + spacing + 10;
...
void App::renderSidebar() {
  ...
  for (auto &btn : buttons) {
    ...
    else if (currentMode == btn.mode && btn.mode != AppMode::NONE)
...
void App::renderStatusBar() {
  ...
  glBegin(GL_QUADS);
  glVertex2i(SIDEBAR_WIDTH, WINDOW_HEIGHT - 35);
  glVertex2i(WINDOW_WIDTH, WINDOW_HEIGHT - 35);
  ...
  ss << "ALGO: " << stats.algoName << " | PIXELS: " << stats.pixelCount
     << " | TIME: " << stats.executionTime << "us | POS: (" << mousePos.x << ","
     << mousePos.y << ")";
...
void App::handleButtonClick(Button &btn) {
  if (btn.label == "Undo (Z)")
    undo();
  else if (btn.label == "Redo (Y)")
    redo();
  else if (btn.label == "Compare")
    toggleSplitScreen();
    ...
    for (auto &s : shapes)
      s->reflect("Y");
  } else if (btn.label == "Reflect O") {
    for (auto &s : shapes)
      s->reflect("Origin");
  } else if (btn.label == "Shear (0.2)") {
    for (auto &s : shapes)
      s->shear(0.2f, 0.0f);
  } else if (btn.label == "S-H Clip") {
    ...
    if (x < SIDEBAR_WIDTH) {
      if (button == GLUT_LEFT_BUTTON) {
        for (auto &btn : app.buttons)
          if (x >= btn.x && x <= btn.x + btn.w && y >= btn.y &&
              y <= btn.y + btn.h)
      ...
      if (button == GLUT_LEFT_BUTTON) {
        if (app.currentMode == AppMode::CLIPPING_POLYGON) {
          ...
          app.isDrawing = true;
                   ...
                   app.currentMode == AppMode::CURVE_BSPLINE) {
          app.inputPoints.push_back({x, y});
          if (app.inputPoints.size() == 4) {
            if (app.currentMode == AppMode::CURVE_BEZIER)
              ...
              app.shapes.push_back(std::make_unique<BezierShape>(
                  app.inputPoints, APP_COLOR_RED, "BSpline"));
          ...
          if (!app.isDrawing) {
            ...
            if (app.currentMode == AppMode::DRAW_DDA ||
                app.currentMode == AppMode::DRAW_BRESENHAM)
              app.shapes.push_back(std::make_unique<LineShape>(
                  app.inputPoints[0], Point(x, y), APP_COLOR_WHITE,
                  (app.currentMode == AppMode::DRAW_DDA ? "DDA"
                                                        : "Bresenham")));
            else if (app.currentMode == AppMode::DRAW_CIRCLE_MIDPOINT ||
                     app.currentMode == AppMode::DRAW_CIRCLE_BRESENHAM)
              app.shapes.push_back(std::make_unique<CircleShape>(
                  app.inputPoints[0],
                  (int)std::sqrt(std::pow(x - app.inputPoints[0].x, 2) +
                                 std::pow(y - app.inputPoints[0].y, 2)),
                  APP_COLOR_WHITE,
                  (app.currentMode == AppMode::DRAW_CIRCLE_MIDPOINT
                       ? "Midpoint"
                       : "Bresenham")));
            else if (app.currentMode == AppMode::CLIPPING_LINE) {
              auto rect = std::make_unique<ClippingRect>(
                  app.inputPoints[0], Point(x, y), APP_COLOR_GREEN);
            ...
            app.isDrawing = false;
                 ...
                 app.currentMode == AppMode::CLIPPING_POLYGON) {
        if (app.inputPoints.size() >= 3) {
          app.shapes.push_back(
              std::make_unique<PolygonShape>(app.inputPoints, APP_COLOR_WHITE));
        }
        app.inputPoints.clear();
        app.isDrawing = false;
      }
...
void App::undo() {
  if (!shapes.empty()) {
    if (shapes.back().get() == activeClipWindow)
      activeClipWindow = nullptr;
...
void App::redo() {
  if (!redoStack.empty()) {
    if (redoStack.back()->getType() == Shape::Type::CLIP_RECT)
      activeClipWindow = static_cast<ClippingRect *>(redoStack.back().get());
...
void App::performClipping() {
  ...
  for (auto &shape : shapes) {
    if (shape->getType() == Shape::Type::LINE) {
      ...
      if (Clipping::cohenSutherlandClip(x1, y1, x2, y2, xmin, ymin, xmax,
                                        ymax)) {
        line->p1 = {x1, y1};
        line->p2 = {x2, y2};
      } else {
...
void App::performPolygonClipping() {
  ...
  for (auto &shape : shapes) {
    if (shape->getType() == Shape::Type::POLYGON) {
      ...
      poly->vertices = Clipping::sutherlandHodgmanClip(poly->vertices, xmin,
                                                       ymin, xmax, ymax);
    }
  }
}
...
void App::loadScene() {
  ...
  for (auto &s : shapes)
    if (s->getType() == Shape::Type::CLIP_RECT)
      activeClipWindow = static_cast<ClippingRect *>(s.get());
}
